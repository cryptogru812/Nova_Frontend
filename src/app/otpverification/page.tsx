...
const OtpVerification: React.FC = () => {
  ...
  const { resendOtp, verifyEmail } = useApi()
  ...
  const handleResendOTP = async () => {
    try {
      if (email) {
        const { data } = await resendOtp.mutateAsync({ email })
        if (data && data.success) {
          toast.success(data.message, {
            position: toast.POSITION.TOP_RIGHT,
          })
            ...
            <div className="gap flex h-full flex-col items-center justify-between gap-4">
              <div className="gap flex flex-col justify-center gap-4 xsm:p-[56px]">
                ...
                <div className=" flex w-full flex-col gap-4">
                  <Button className="rounded-xs !bg-button-gradient p-2 px-2" onClick={handleVerification}>
                    Verify
                  </Button>
                </div>
              </div>
